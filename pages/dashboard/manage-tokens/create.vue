				...
				<form @submit.prevent="handelSubmit">
					<div class="custom-card-body">
						<div class="row gy-4">
							...
							<FormGroupPhoto col="col-md-6" v-model="form.logo" message="Size: 95x95" :error="form.errors.get('logo')" label="Token Logo" />
							<FormGroupPhoto col="col-md-6" v-model="form.feature_image" message="Size: 445x370" :error="form.errors.get('feature_image')" label="Feature Image" />
							...
							<FormGroupPhoto col="col-md-4" v-model="form.company_logo" message="Size: 100x100" :error="form.errors.get('company_logo')" label="Company Logo" />
							...
							<table class="table table-bordered col-md-12">
								<thead>
									<tr>
										...
										<th>
											<button type="button" @click="addDistribution" class="gradient-btn px-3 py-1 fs-6">Add</button>
										</th>
									</tr>
								</thead>
								<tbody>
									<tr v-for="(distribution, index) in form.distributions" :key="index">
										...
										<td>
											<button type="button" @click="removeDistribution(index)" class="gradient-btn py-1 px-2 fs-6">Remove</button>
										</td>
									</tr>
								</tbody>
							</table>
							<FormGroupPhoto col="col-md-6" v-model="form.distribution_image" message="Size: 548x517" :error="form.errors.get('distribution_image')" label="Distribution Image" />
...
<script setup>
	...
	const handelSubmit = async () => {
		await form.submit('/manage-tokens').then((res) => {
			form.reset();
